trigger CalculatesSLADateTimeDataImport on Case (before insert, before update) 
{
    ...
    {   
        if(c.AddedDate__c == null)
            dateCreated= System.now();
        else 
            dateCreated = c.AddedDate__c;  
        if(c.SLA_number__c != null && c.SLA_number__c != 'None' && c.SLA_date__c == null)    {
            sla = Integer.ValueOf(c.SLA_number__c); 
            dateToUpdate = BusinessHours.add('01mG00000007OCeIAM', dateCreated , sla*9* 60 * 60 * 1000L);
            ...
            dateToUpdate = BusinessHours.add('01mG00000007OCeIAM', dateToUpdate , noOfHolidays*9* 60 * 60 * 1000L);
